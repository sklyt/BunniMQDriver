...
export default class Bunnymq {
    ...
    handleCommandResponse(packet) {
        ...
        switch (this._command.type) {
            ...
            case "Publish":
                if (packet.res == ERROR) {
                    this._command.cb({status: packet.res, error: new Error(`Publish ERROR: ${this._command.payload}`)})
                } else {
                    this._command.cb({status :packet.res, message: this._command.payload})
                }
                break;
            case "Ack":
